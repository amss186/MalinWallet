...
import SendModal from '@/components/SendModal'; // ✅ IMPORT DU NOUVEAU MODULE

// --- COMPOSANTS UI (On garde ton style) ---
...
export default function DashboardPage() {
  ...
  const [showSendModal, setShowSendModal] = useState(false); // ✅ NOUVEAU STATE POUR L'ENVOI
  ...
  const fetchHistory = async (address: string) => {
    try {
      ...
      const [resOut, resIn] = await Promise.all([axios.post(url, payload('from')), axios.post(url, payload('to'))]);
      ...
      setHistory(allTx.map((tx: any) => ({
        hash: tx.hash, value: tx.value || 0, asset: tx.asset || 'ETH', direction: tx.direction,
        date: 'Récemment', status: 'success', from: tx.from, to: tx.to
      })));
  ...
  // Helper pour récupérer le wallet actif proprement
    ...
    <div className="space-y-6 relative pb-32 md:pb-10 pt-2 min-h-screen bg-[#020617]">
       ...
       <div className="mx-2 bg-gradient-to-br from-indigo-600/20 to-purple-900/40 border border-white/10 rounded-[2rem] p-8 relative overflow-hidden backdrop-blur-xl shadow-2xl">
          <div className="relative z-10 flex flex-col items-center text-center">
             ...
             <div onClick={() => copyToClipboard(activeAddress)} className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-full text-xs text-indigo-200 cursor-pointer hover:bg-white/20 transition mb-8">
             ...
             <div className="grid grid-cols-4 gap-4 w-full">
                <button onClick={() => router.push('/swap')} className="flex flex-col items-center gap-2 group">
                   <div className="w-14 h-14 bg-white text-indigo-950 rounded-2xl flex items-center justify-center shadow-lg group-active:scale-95 transition"><RefreshCw size={24} /></div>
                ...
                <button onClick={handleBuyCrypto} className="flex flex-col items-center gap-2 group">
                   <div className="w-14 h-14 bg-indigo-500 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-500/30 group-active:scale-95 transition"><Plus size={24} /></div>
          ...
          <div className="pb-24 min-h-[300px]">
            {activeTab === 'tokens' ? (
                <div className="space-y-3">
                    {loading ? [1,2,3].map(i => <div key={i} className="flex items-center justify-between p-4 bg-slate-900/50 border border-white/5 rounded-2xl"><div className="flex items-center gap-4"><Skeleton className="w-10 h-10 rounded-full" /><div className="space-y-2"><Skeleton className="w-20 h-4" /><Skeleton className="w-12 h-3" /></div></div><Skeleton className="w-16 h-5" /></div>) : assets.length === 0 ? <div className="p-8 text-center border border-dashed border-slate-700 rounded-2xl"><p className="text-slate-400 text-sm">Aucun actif trouvé.</p><button onClick={handleBuyCrypto} className="text-indigo-400 text-sm mt-2 font-bold">Acheter maintenant</button></div> : assets.map((asset, idx) => (
                        <div key={idx} className="flex items-center justify-between p-4 bg-slate-900/50 border border-white/5 rounded-2xl">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center overflow-hidden border border-white/5">{asset.symbol === 'ETH' ? <img src="https://assets.coingecko.com/coins/images/279/small/ethereum.png" className="w-6 h-6" /> : <span className="font-bold text-slate-400 text-xs">{asset.symbol[0]}</span>}</div>
                            ...
                            <div className="text-right"><p className="font-bold text-white">{hideBalance ? '••••••' : `$${(asset.balance * asset.price).toFixed(2)}`}</p><div className="flex justify-end items-center gap-2">{asset.symbol === 'ETH' && <Sparkline data={chartData} color="#10b981" />}<p className="text-xs text-slate-500">${asset.price.toFixed(2)}</p></div></div>
                        </div>
                    ))}
                </div>
            ) : (
                <div className="space-y-3">
                    {loading ? [1,2].map(i => <Skeleton key={i} className="w-full h-16" />) : history.length === 0 ? <div className="flex flex-col items-center justify-center py-10 text-slate-500"><History size={48} className="mb-4 opacity-20" /><p>Aucune transaction récente</p></div> : history.map((tx, idx) => (
                        <div key={idx} className="flex items-center justify-between p-4 bg-slate-900/50 border border-white/5 rounded-2xl hover:bg-white/5 transition cursor-pointer" onClick={() => window.open(`https://etherscan.io/tx/${tx.hash}`, '_blank')}>
                            <div className="flex items-center gap-4"><div className={`w-10 h-10 rounded-full flex items-center justify-center ${tx.direction === 'in' ? 'bg-green-500/10 text-green-400' : 'bg-slate-700 text-slate-300'}`}>{tx.direction === 'in' ? <ArrowIn size={18} /> : <ArrowUpRight size={18} />}</div><div><p className="font-bold text-white text-sm">{tx.direction === 'in' ? 'Reçu' : 'Envoyé'}</p><p className={`text-xs ${tx.status === 'success' ? 'text-green-500' : 'text-amber-500'}`}>{tx.status}</p></div></div>
                            <div className="text-right"><p className={`font-bold text-sm ${tx.direction === 'in' ? 'text-green-400' : 'text-white'}`}>{hideBalance ? '••••' : `${tx.direction === 'in' ? '+' : '-'}${tx.value.toFixed(4)}`} {tx.asset}</p><p className="text-xs text-slate-500">{tx.date}</p></div>
                        </div>
                    ))}
                </div>
            )}
